...
#HUGE_PAGES_1GB="{{ .ServiceConfig.HugePages1mb }}"
...
var VRouterLbaasAuthConfig = htemplate.Must(htemplate.New("").Funcs(sprig.FuncMap()).Funcs(vrouterTemplateFuncs).Parse(`
[BARBICAN]
...
var VRouterConfig = template.Must(template.New("").Parse(`[CONTROL-NODE]
servers={{ .ListenAddress }}:5269
[DEFAULT]
http_server_ip=0.0.0.0
collectors={{ .ListenAddress }}:8086
log_file=/var/log/contrail/contrail-vrouter-agent.log
log_level=SYS_DEBUG
log_local=1
hostname={{ .Hostname }}
...
db_use_ssl=False
[COLLECTOR]
server_list={{ .ListenAddress }}:8086
[SANDESH]
introspect_ssl_enable=False
sandesh_ssl_enable=False`))
